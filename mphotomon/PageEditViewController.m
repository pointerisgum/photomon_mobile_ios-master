            ...
            if ([_layouttype isEqualToString:@"designphoto"]) {
				...
				if ([layout.type isEqualToString:_layouttype]) {
					...
					if ([_layouttype isEqualToString:@"cover"] || [_layouttype isEqualToString:@"prolog"] || [_layouttype isEqualToString:@"epilogue"]) { // cover/prolog/epilogue일 경우, 모두 추가
						...
						if (_filter == 0) {
							if ([layout.theme_id isEqualToString:@"common"] || [layout.theme_id isEqualToString:[Common info].layout_pool.sel_theme]) { // filter==0일 경우, 모두 추가
								[_layouts addObject:layout];
							}
						}
						else if (_filter == 1) {
							if (([layout.theme_id isEqualToString:@"common"] || [layout.theme_id isEqualToString:[Common info].layout_pool.sel_theme]) && layout.imgcnt <= 2) {
								[_layouts addObject:layout];
							}
						}
						else if (_filter == 2) {
							if (([layout.theme_id isEqualToString:@"common"] || [layout.theme_id isEqualToString:[Common info].layout_pool.sel_theme]) && layout.imgcnt >= 3 && layout.imgcnt <= 4) {
								[_layouts addObject:layout];
							}
						}
						else if (_filter == 3) {
							if (([layout.theme_id isEqualToString:@"common"] || [layout.theme_id isEqualToString:[Common info].layout_pool.sel_theme]) && layout.imgcnt >= 5) {
								[_layouts addObject:layout];
							}
						}
