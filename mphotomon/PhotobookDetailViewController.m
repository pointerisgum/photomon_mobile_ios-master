//
//  PhotobookDetailViewController.m
//  PHOTOMON
//
...
@interface PhotobookDetailViewController (){
...
@implementation PhotobookDetailViewController
...
- (void)viewDidLoad {
    ...
    _addedConstraints = [[NSMutableArray alloc]init];
	...
	if(_book_info == nil && [_selected_theme.book_infos count] >= 1) {
		...
		if([tbookinfo.productcode isEqualToString:@"300180"] || [tbookinfo.productcode isEqualToString:@"120069"] || [[Common info] isGudakBook:tbookinfo.productcode]) {
            ...
            if([[Common info].deeplink_url containsString:@"mobile_cataloga4photobook"]){
                _book_info = _selected_theme.book_infos[1];
...
- (void)setConstraints:(BOOL)isViewDidAppear{
    ...
    if([_selected_theme.theme1_id isEqualToString:@"catalog"] || [_book_info.productcode isEqualToString:@"120069"]) {
    ...
    {
        _covertype_title.hidden = NO;
        _covertype.hidden = NO;
...
- (IBAction)selectBookSize:(id)sender {
    ...
    for (int i = 0; i < _selected_theme.book_infos.count; i++) {
        ...
        NSString *bsize = bi.booksize;
        if([_selected_theme.theme1_id isEqualToString:@"skinny"] || [_selected_theme.theme1_id isEqualToString:@"catalog"] ){
            bsize = _selected_theme.book_sizes[i];
            bsize = [bsize stringByAppendingString:@" "];
        }
        
        NSString *size_title = [NSString stringWithFormat:@"%@ (%@)", bsize, bi.cm];
...
- (void)removeAddedConstraints {
   [_content_view removeConstraints:_addedConstraints];
   [_addedConstraints removeAllObjects];
}
@end
