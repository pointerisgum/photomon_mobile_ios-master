...
@interface GiftMugDesignViewController ()

@end

@implementation GiftMugDesignViewController
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    [Analysis firAnalyticsWithScreenName:@"GiftMugDesign" ScreenClass:[self.classForCoder description]];
}
    ...
    if ([[Common info].photobook_theme loadTheme:_product_type]) {
        if (_product_type == PRODUCT_MUG) {
            ...
            for (int i = theme_count-1; i >= 0; i--) {
                ...
                if (![theme.theme1_id isEqualToString:@"mug"]) {
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"GiftMugDetailSegue"]) {
        GiftMugDetailViewController *vc = [segue destinationViewController];
        if (vc) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    UICollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"GiftMugCell" forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(!bOpen){
        ...
        if (_product_type == PRODUCT_MUG) {
            [self performSegueWithIdentifier:@"GiftMugDetailSegue" sender:self];
        }
    }
}
