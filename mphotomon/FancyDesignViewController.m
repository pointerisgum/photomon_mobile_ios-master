...
@interface FancyDesignViewController ()

@end

@implementation FancyDesignViewController
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    [Analysis firAnalyticsWithScreenName:@"FancyDesign" ScreenClass:[self.classForCoder description]];
}
...
- (void)loadFancyTheme {
    ...
    if ([[Common info].photobook_theme loadTheme:_product_type]) {
        if (_product_type == PRODUCT_NAMESTICKER) {
            ...
            for (int i = theme_count-1; i >= 0; i--) {
                ...
                if (![theme.theme1_id isEqualToString:@"namesticker"]) {
            ...
            for (int i = theme_count-1; i >= 0; i--) {
                Theme *theme = [Common info].photobook_theme.themes[i];
                if (![theme.theme1_id isEqualToString:@"divisionsticker"]) {
                    [[Common info].photobook_theme.themes removeObjectAtIndex:i];
                }
            }
        }
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"FancyNameStickerDetailSegue"]) {
        FancyDetailViewController *vc = [segue destinationViewController];
        if (vc) {
    ...
    else if([segue.identifier isEqualToString:@"FancyDivisionStickerSegue"]) {
        /*FancyDivisionStickerDetailViewController *vc = [segue destinationViewController];
        if (vc) {
            NSIndexPath *indexPath = [[_collection_view indexPathsForSelectedItems] lastObject];
            vc.selected_theme = [Common info].photobook_theme.themes[indexPath.row];
            [vc updateTheme];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    UICollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"FancyNameStickerCell" forIndexPath:indexPath];
...
//            NSString *url = [fullpath stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
            ...
            UILabel *label1 = (UILabel *)[cell viewWithTag:102];
            label1.text = theme.theme_name;
            if (_product_type == PRODUCT_NAMESTICKER || _product_type == PRODUCT_DIVISIONSTICKER) {
                UILabel *label2 = (UILabel *)[cell viewWithTag:103];
                label2.text = [NSString stringWithFormat:@"%@원", price];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(!bOpen){
        ...
        if (_product_type == PRODUCT_NAMESTICKER) {
            [self performSegueWithIdentifier:@"FancyNameStickerDetailSegue" sender:self];
        }
        else if (_product_type == PRODUCT_DIVISIONSTICKER) {
            ...
            FancyDivisionDetailViewController *vc = [sb instantiateViewControllerWithIdentifier:@"fancyDivisionDetailView"];
             if (vc) {
             NSIndexPath *indexPath = [[_collection_view indexPathsForSelectedItems] lastObject];
             vc.selected_theme = [Common info].photobook_theme.themes[indexPath.row];
             [vc updateTheme];
