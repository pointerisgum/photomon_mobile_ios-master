//
//  PhotobookV2BackgroundViewController.m
//  PHOTOMON
//
//  Created by kim kihwan on 2019. 8. 29..
...
@interface PhotobookV2BackgroundViewController ()

@end

@implementation PhotobookV2BackgroundViewController
...
- (void)viewDidLoad {
    ...
    for (Background *background in [Common info].background_pool.backgrounds) {
        ...
        NSString *fullpath = background.thumbnail;
                ...
                if ([_sel_pagetype isEqualToString:@"cover"]) {       // cover일 경우, 모두 추가
                    if (_filter == 1) {
                        if ([background.theme_id hasPrefix:@"common"]) { //if (background.skinfilename.length <= 0) {
                            ...
                            //if (background.skinfilename.length > 0) {
                            [_backgrounds addObject:background];
                        }
                    }
                    else {
