...
@interface CardEditTextViewController ()

@end

@implementation CardEditTextViewController
...
- (void)viewDidLoad {
    ...
    CGFloat widthratio = boundSize.width / _edittext_width;
    _constraint_edittext_width.constant = _edittext_width * widthratio;
    _constraint_edittext_height.constant = _edittext_height * widthratio;

    float fontsize = _fontsize;
	fontsize = fontsize * 1.00f * widthratio;
...
-(CGSize)sizeOfString:(NSString*)string constrainedToWidth:(CGFloat)width font:(UIFont*)font {
...
- (float)doesFit:(UITextView*)textView string:(NSString *)myString range:(NSRange) range;
{
    ...
    if ( textHeight >= viewHeight - 1 && fabsf(textHeight - viewHeight) > 5) {
        NSLog(@" fabsf(textHeight - viewHeight) > 1");
        return NO;
    } else
        return YES;
...
//        return YES;
    ...
    /*NSString *newText = [textView.text stringByReplacingCharactersInRange:range withString:myString];
...
{
	if(!moveup && _fontsize*3 < _edittext_height)
	{
		[self setViewMovedUp:YES];
	}
}
