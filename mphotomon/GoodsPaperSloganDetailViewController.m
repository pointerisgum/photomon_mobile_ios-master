//
//  GoodsPaperSloganDetailViewController.m
//  PHOTOMON
//
...
#import "GoodsPaperSloganDetailViewController.h"
...
@interface GoodsPaperSloganDetailViewController ()

@property NSInteger sel_coating_idx;

@end

@implementation GoodsPaperSloganDetailViewController

- (void)viewDidLoad {

    [super viewDidLoad];
    [Analysis log:@"PaperSloganDetail"];
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    [Analysis firAnalyticsWithScreenName:@"PaperSloganDetail" ScreenClass:[self.classForCoder description]];
    ...
    [self setLabelTopConstraint:_booksize pos:start_y + space * step];
//    [self setButtonTopConstraint:_booksize_icon pos:start_y + space * step++];
    ...
    if([_book_info.productcode isEqualToString:@"120069"]) {
        _default_components_title.hidden = YES;
        _default_components.hidden = YES;
    }
    else
    {
        [self setLabelTopConstraint:_default_components_title pos:start_y + space * step];
        [self setLabelTopConstraint:_default_components pos:start_y + space * step++];
    }
...
- (IBAction)selectInnerCort:(id)sender {
    ...
    for (int i = 0; i < _selected_theme.sel_coatings.count; i++) {
        ...
        alert_action = [UIAlertAction actionWithTitle:sel_item.comment style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
            ...
            if (i == 0) {
                for (BookInfo *tbookinfo in self.selected_theme.book_infos) {
                    if ([tbookinfo.productcode isEqualToString:@"300221"] || [tbookinfo.productcode isEqualToString:@"300225"] || [tbookinfo.productcode isEqualToString:@"300223"]) {
                        tbookinfo.productcode = [NSString stringWithFormat:@"%ld", [tbookinfo.productcode integerValue] - 20];
                    }
                }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_selected_theme != nil) {
        ...
        NSString *size_title = [NSString stringWithFormat:@"%@", _book_info.booksize];
        size_title = [self getBookSizeTitle:size_title];
        _booksize.text = size_title;
        ...
        _default_components.text = [_selected_theme.sel_options[0] comment];
...
- (IBAction)selectOption:(id)sender {
    ...
    for (int i = 0; i < _selected_theme.sel_options.count; i++) {
        ...
        alert_action = [UIAlertAction actionWithTitle:item.comment style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
            ...
            [self.coverlogo setTitle:item.comment forState:UIControlStateNormal];
            [vc dismissViewControllerAnimated:YES completion:nil];
