//
//  GiftPhonecaseDesignViewController.m
//  PHOTOMON
//
...
@interface GiftPhonecaseDesignViewController ()

@end

@implementation GiftPhonecaseDesignViewController
...
- (void)viewDidLoad {
    ...
    if([Common info].deeplink_url != nil) {
        if ( [[Common info].deeplink_url rangeOfString:@"mobile_phonecase"].location != NSNotFound ) {
            [self.navigationController setNavigationBarHidden:NO animated:NO];
            [Common info].deeplink_url= nil;
        }
    }

    [Analysis log:@"GiftPhonecaseDesign"];
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    [Analysis firAnalyticsWithScreenName:@"GiftPhonecaseDesign" ScreenClass:[self.classForCoder description]];
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"GiftPhonecaseDetailSegue"]) {
        GiftPhonecaseDetailViewController *vc = [segue destinationViewController];
        if (vc) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    UICollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"GiftPhonecaseCell" forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(!bOpen){
        ...
        if (_product_type == PRODUCT_PHONECASE) {
            [self performSegueWithIdentifier:@"GiftPhonecaseDetailSegue" sender:self];
        }
    }
}
