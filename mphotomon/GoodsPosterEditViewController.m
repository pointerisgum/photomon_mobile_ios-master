//
//  GoodsPosterEditViewController.m
//  PHOTOMON
//
...
@interface GoodsPosterEditViewController () {
...
@implementation GoodsPosterEditViewController

- (void)viewDidLoad {
    //NSLog(@"-- [Common info].photobook.AddVal11 : %@", [Common info].photobook.AddVal11);
    [super viewDidLoad];
    [Analysis log:@"PosterEdit"];
    _thread = nil;
...
- (void)startFillPhotoThread {
    _progressView = [[ProgressView alloc]initWithTitle:@"포스터 구성 중"];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"PhotoEditSegue"]) {
        ...
        if (vc) {
            ...
            [[Common info].layout_pool loadLayouts:[Common info].photobook.ProductSize Theme:@"" ProductType:PRODUCT_POSTER];
            [vc updateLayouts];
        }
...
- (void)photobookProcess:(int)count TotalCount:(int)total_count {
    NSString *progress_str = [NSString stringWithFormat:@"포스터 구성 중 (%d/%d)", count, total_count];
