//
//  GoodsPaperSloganDesignViewController.m
//  PHOTOMON
//
...
@interface GoodsPaperSloganDesignViewController () {
    BOOL retainViewController;
}

@end

@implementation GoodsPaperSloganDesignViewController
...
- (void)viewDidLoad {
    ...
    [Common info].paperslogan_root_controller = self;

    [Analysis log:@"PaperSlogan"];

    [self loadPaperSloganTheme];
}
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    [Analysis firAnalyticsWithScreenName:@"PaperSlogan" ScreenClass:[self.classForCoder description]];
}

- (void)viewDidDisappear:(BOOL)animated {
    [super viewDidDisappear:animated];
    if (!retainViewController) {
        [Common info].paperslogan_root_controller = nil;
    }
}
    ...
    if ([[Common info].photobook_theme loadTheme:_product_type]) {
        if (_product_type == PRODUCT_PAPERSLOGAN) {
            ...
            for (int i = theme_count-1; i >= 0; i--) {
                ...
                if (![theme.theme1_id isEqualToString:@"Paperslogan"]) {
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if ([segue.identifier isEqualToString:@"PaperSloganDetailSegue"]) {
        GoodsPaperSloganDetailViewController *vc = [segue destinationViewController];
        if (vc) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    UICollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"GoodsPosterCell" forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(!bOpen){
        ...
        if (_product_type == PRODUCT_PAPERSLOGAN) {
            [self performSegueWithIdentifier:@"PaperSloganDetailSegue" sender:self];
        }
    }
}
