//
//  GoodsPosterDesignViewController.m
//  PHOTOMON
//
...
@interface GoodsPosterDesignViewController () {
    BOOL retainViewController;
}

@end

@implementation GoodsPosterDesignViewController
...
- (void)viewDidLoad {
    ...
    [Common info].poster_root_controller = self;

    [Analysis log:@"Poster"];

    [self loadPosterTheme];
}
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    [Analysis firAnalyticsWithScreenName:@"Poster" ScreenClass:[self.classForCoder description]];
}

- (void)viewDidDisappear:(BOOL)animated {
    [super viewDidDisappear:animated];
    if (!retainViewController) {
        [Common info].poster_root_controller = nil;
    }
}
    ...
    if ([[Common info].photobook_theme loadTheme:_product_type]) {
        if (_product_type == PRODUCT_POSTER) {
            ...
            for (int i = theme_count-1; i >= 0; i--) {
                ...
                if (![theme.theme1_id isEqualToString:@"poster"]) {
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if ([segue.identifier isEqualToString:@"PosterDetailSegue"]) {
        GoodsPosterDetailViewController *vc = [segue destinationViewController];
        if (vc) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(!bOpen){
        ...
        if (_product_type == PRODUCT_POSTER) {
            [self performSegueWithIdentifier:@"PosterDetailSegue" sender:self];
        }
    }
}
