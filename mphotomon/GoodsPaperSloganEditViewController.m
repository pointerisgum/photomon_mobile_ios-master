//
//  GoodsPaperSloganEditViewController.m
//  PHOTOMON
//
...
@interface GoodsPaperSloganEditViewController () {
...
@implementation GoodsPaperSloganEditViewController

- (void)viewDidLoad {
    //NSLog(@"-- [Common info].photobook.AddVal11 : %@", [Common info].photobook.AddVal11);
    [super viewDidLoad];
    [Analysis log:@"PaperSloganEdit"];
    _thread = nil;
...
//              scrollPosition:UICollectionViewScrollPositionCenteredHorizontally];
    is_selecting_nav_item = NO;
    originalTitle = self.navigationItem.title;
}
...
-(void)appWillResignActive:(NSNotification*)noti {
    if (self.navigationController.topViewController == self) {
        ...
        if(!(_is_new == YES && is_photo_done == NO)) {
            [[Common info].photobook saveFileWithPageCount:[self getPageCountTotal]];
...
- (void)startFillPhotoThread {
    _progressView = [[ProgressView alloc]initWithTitle:@"종이슬로건 구성 중"];
...
- (void)singleTapPiece:(id)sender {
    ...
    if (indexPath && state == UIGestureRecognizerStateEnded) {
        ...
        CGPoint touchedPoint = [_collection_view convertPoint:location toView:bkimageview];

        isUpSelected = touchedPoint.y + 20 < _collection_view.frame.size.height / 2;
        /*
        [cell setNeedsLayout];
        [cell layoutIfNeeded];
        ...
        */
        ...
        [_nav_view reloadItemsAtIndexPaths:@[indexPath]];
        ...
        for (int i = 0; i < 2; i++) {
            Layer *layer = nil;
            {
                ...
                if (i == 1) {
                    point_page.y -= 300 * [Common info].photobook.heightScale;
                }
                //CGPoint point_page = [cell.contentView convertPoint:location toView:bkimageview];
                layer = [[Common info].photobook getLayerOfPaperSlogan:indexPath.row * 2 + i FromPoint:point_page];
            }
            if(layer == nil) {
                [_collection_view reloadData];
                CGPoint point_page = [_collection_view convertPoint:location toView:bkimageview];
...
{
    if ([self getPageCountTotal] + 1 > [Common info].photobook.MaxPage) {
        [self.view makeToast:@"더이상 페이지를 추가할 수 없습니다."];
    } else {
        NSIndexPath *indexPath;
        indexPath = [self.nav_view indexPathForItemAtPoint:[self.nav_view convertPoint:btn.center fromView:btn.superview]];
    ...
    CGRect frame;
    {
        ...
        @try {
            for (UIView *aView in [bkimageview subviews]){
                if ([aView isKindOfClass:[UIImageView class]] || aView.tag == 10001 || aView.tag == 10002){
...
//        CGFloat width = (height * page_rect.size.width) / page_rect.size.height;
        ...
        CGFloat width = collectionView.frame.size.width - spacing * 2;
        Page* page = (Page*)[Common info].photobook.pages[indexPath.row];
        CGFloat heightRatio = (CGFloat) page.PageHeight / page.PageWidth;
        CGFloat height = width * heightRatio;
        CGFloat heightScale = height / page.PageHeight;
        CGFloat scale = collectionView == _collection_view ? 1.f : 0.5f;
...
//            frame.size = CGSizeMake(width, height);
        frame.size = CGSizeMake(width * scale, (height * 2 + (collectionView == _collection_view ? 210 : 110) * heightScale) * scale);
        ...
        [Common info].photobook.heightScale = heightScale;
        [Common info].photobook.widthScale = width / page.PageWidth;
    }
    if(collectionView == _collection_view)
        [[Common info].photobook composePaperSloganPage:indexPath.row * 2 ParentView:bkimageview IncludeBg:NO IsEdit:YES IsThumbnail:NO];
    else
        [[Common info].photobook composePaperSloganPage:indexPath.row * 2 ParentView:bkimageview IncludeBg:NO IsEdit:NO IsThumbnail:NO];

    UILabel *frontLabel = [bkimageview viewWithTag:10001];
    UILabel *backLabel = [bkimageview viewWithTag:10002];

    if (frontLabel != nil) {
        if (_selected_page == indexPath.row && isUpSelected) {
            frontLabel.font = [UIFont boldSystemFontOfSize:13];
        } else {
            frontLabel.font = [UIFont systemFontOfSize:13];
        }
        if (_selected_page == indexPath.row && !isUpSelected) {
            backLabel.font = [UIFont boldSystemFontOfSize:13];
        } else {
            backLabel.font = [UIFont systemFontOfSize:13];
        }
    }

    UIImageView *frontPageImageView = [bkimageview viewWithTag:5001];
    UIImageView *backPageImageView = [bkimageview viewWithTag:5002];

    if (_selected_page == indexPath.row && isUpSelected) {
        frontPageImageView.layer.borderWidth = 3;
        frontPageImageView.layer.borderColor = [UIColor colorFromHexString:@"#ff8d1d"].CGColor;
    } else {
        frontPageImageView.layer.borderWidth = 0;
    }
    if (_selected_page == indexPath.row && !isUpSelected) {
        backPageImageView.layer.borderWidth = 3;
        backPageImageView.layer.borderColor = [UIColor colorFromHexString:@"#ff8d1d"].CGColor;
    } else {
        backPageImageView.layer.borderWidth = 0;
    }
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"PhotoEditSegue"]) {
        ...
        if (vc) {
            ...
            Page* tpage = [Common info].photobook.pages[_selected_page * 2 + (isUpSelected ? 0 : 1)];
            vc.layouttype = @"designphoto";
            [[Common info].layout_pool loadLayouts:[Common info].photobook.ProductSize Theme:@"" ProductType:PRODUCT_PAPERSLOGAN];
            [vc updateLayouts];
        }
    } else if ([segue.identifier isEqualToString:@"CardEditTextSegue"]) {
...
- (IBAction)addPage:(id)sender {
    if ([self getPageCountTotal] + 1 > [Common info].photobook.MaxPage) {
        ...
        NSInteger pageno = [Common info].photobook.pages.count / 2;
        Page *default_page = [[Common info].photobook.pages[0] copy]; // 첫번째 페이지 복사
        Page *default_page2 = [[Common info].photobook.pages[1] copy];
        ...
        */
        ...
        if ([[Common info].photobook insertPage:pageno * 2 - 1 FromDefaultPage:default_page]) {
            [[Common info].photobook insertPage:pageno * 2 FromDefaultPage:default_page2];
            [_nav_item_cnt addObject:@"1"];
...
- (void)photobookProcess:(int)count TotalCount:(int)total_count {
    NSString *progress_str = [NSString stringWithFormat:@"종이슬로건 구성 중 (%d/%d)", count, total_count];
...
- (void)changeLayout:(Layout *)layout {
    if (layout != nil && _selected_page >= 0) {
        ...
        [[Common info].photobook changeLayout:_selected_page * 2 + (isUpSelected ? 0 : 1) From:layout];
...
- (int) getPageCountTotal {
    int sum = 0;

    for (NSString *count in _nav_item_cnt) {
        sum += [count intValue];
    }
...
- (void) updateTitle {
    self.title = [[NSString alloc] initWithFormat:@"%@(%d/%d)", originalTitle, [self getPageCountTotal], [Common info].photobook.MaxPage];
}
