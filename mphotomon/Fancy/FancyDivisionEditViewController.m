//
//  FancyDivisionEditViewController.m
//  PHOTOMON
//
...
@interface FancyDivisionEditViewController (){
    BOOL is_selecting_nav_item;
    BOOL is_photo_done;
}
@end

@implementation FancyDivisionEditViewController
...
- (void)viewDidLoad {
    ...
    if (_is_new) {
        ...
        [[Common info].layout_pool loadLayouts:[Common info].photobook.ProductSize Theme:@"" ProductType:PRODUCT_DIVISIONSTICKER];
        //기본 레이아웃적용은 제외함.(이것때문에 첫번째 페이지 정보가 날라감...)
        //(프로덕트의 레이아웃 정보가 없는 경우에 nil객체가 되는데 이를 changeLayout 에서 변경시켜버림)
        //Layout *defaultLayout = [[Common info].layout_pool getDefaultLayout:PRODUCT_DIVISIONSTICKER];
        ...
        for( int pageCount=0; pageCount < [Common info].photobook.pages.count ; pageCount++ ){
...
- (void)startFillPhotoThread {
	_progressView = [[ProgressView alloc]initWithTitle:@"구성 중"];
