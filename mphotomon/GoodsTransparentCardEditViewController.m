//
//  GoodsTransparentCardEditViewController.m
//  PHOTOMON
//
...
@interface GoodsTransparentCardEditViewController () {
...
@implementation GoodsTransparentCardEditViewController
...
{
    if ([self getPageCountTotal] + 1 > MAX_PAGE) {
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"PhotoEditSegue"]) {
        ...
        if (vc) {
            ...
            [[Common info].layout_pool loadLayouts:[Common info].photobook.ProductSize Theme:@"" ProductType:PRODUCT_TRANSPARENTCARD];
            [vc updateLayouts];
        }
...
- (void) updateTitle {
    self.title = [[NSString alloc] initWithFormat:@"%@(%d/%d)", originalTitle, [self getPageCountTotal], MAX_PAGE];
}
