//
//  DdukddakDesignSelectViewController.m
//  PHOTOMON
//
...
@interface DdukddakDesignSelectViewController ()<MainTabBarDelegate>
...
@implementation DdukddakDesignSelectViewController
...
- (void)loadPhotobookStyle {
    ...
    if([Ddukddak inst].productSelectedIdx == 0){
        if ([[Common info].photobook_theme loadPhotobookStyleFromUrl:_product_type fromUrl:[Ddukddak inst].photobookDesignListURL]) {
    ...
    else if([Ddukddak inst].productSelectedIdx == 1){
        if ([[Common info].photobook_theme loadPhotobookStyleFromUrl:_product_type fromUrl:[Ddukddak inst].layflatDesignListURL]) {
            if (_product_type == PRODUCT_PHOTOBOOK) {
                int theme_count = (int)[Common info].photobook_theme.themes.count;
                for (int i = theme_count-1; i >= 0; i--) {
                    Theme *theme = [Common info].photobook_theme.themes[i];
            ...
            if (_product_type == PRODUCT_PHOTOBOOK) {
                if ([[Common info] isGudakBook:productcode]) {
                    ...
                    if(selectedTheme.depth1_key.length > 0){
                        ...
                        DdukddakDetailViewController *vc = [self.storyboard  instantiateViewControllerWithIdentifier:@"DdukddakDetailView"];
                        if (vc) {
                            NSIndexPath *indexPath = [[_collection_view indexPathsForSelectedItems] lastObject];
                            
                            //vc.selected_theme = _photobook_themes[indexPath.row];
                            [Ddukddak inst].selectedTheme = _photobook_themes[indexPath.row];
                            
                            NSString *themeUrl =[NSString stringWithFormat:@"%@?depth1=%@&depth2=%@&producttype=%@&svcmode=ddudak",[[Ddukddak inst].themeInfoURL stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]] ,[Ddukddak inst].selectedTheme.depth1_key,[Ddukddak inst].selectedTheme.depth2_key,[Ddukddak inst].productSelectedIdx == 0 ? @"designphotobook" : @"layflat"];
                            
                            [[Common info].photobook_theme loadPhotobookTheme:_product_type  selectedTheme:[Ddukddak inst].selectedTheme themeUrl:themeUrl];
